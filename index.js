...
function complexConvert(data) {
  ...
  // Функція для обробки об'єкта (включаючи вкладені об'єкти)
  function processObject(obj) {
    ...
    for (let key in obj) {
      ...
      if (typeof obj[key] === "number") {
        ...
        // Якщо значення — об'єкт, рекурсивно обробляємо його
        newObj[key] = processObject(obj[key]);
      } else {
        newObj[key] = obj[key]; // Інші типи зберігаємо без змін
      }
    }
  ...
  // Обробляємо об'єкт
  result = processObject(parsedData);
...
function manipulateUrl(url) {
  ...
  return {
    ...
    params: Array.from(urlObj.searchParams), // параметри url як масив пар [ключ, значення]
  };
}
...
function checkURLParameters(url, params) {
  ...
  for (let key of params) {
    ...
    result[key] = objUrl.searchParams.has(key);
  }
...
function getUrlData(url) {
  ...
  return {
    origin: objUrl.origin, // Повне походження
    hostname: objUrl.hostname, // Імя хоста
    port: objUrl.port || null, // Порт, якщо вказаний, або null
    username: objUrl.username || null, // Імя користувача, якщо вказане, або null
    password: objUrl.password || null, // Пароль, якщо вказаний, або null
  };
}
...
function sortUrlParams(url) {
  ...
  const paramsArray = Array.from(objUrl.searchParams.entries());
